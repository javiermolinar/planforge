const EXECUTION_MODES = new Set(["auto", "supervised"]);
const ACCEPTANCE_STATES = new Set(["none", "awaiting", "accepted", "revise_requested"]);
const SCOPE_KINDS = new Set(["none", "implementation", "review", "replan"]);
...
function gateStatusLine(state) {
  ...
  if (acceptanceState === "awaiting") {
    return `PF gate: awaiting scenario acceptance (scope v${Math.max(1, state?.scopeVersion || 1)}${modeLabel}${reviewLabel}${reviewNextLabel}${scopeLabel})`;
  }
  if (acceptanceState === "revise_requested") {
    return `PF gate: revision requested (scope v${Math.max(1, state?.scopeVersion || 1)}${modeLabel}${reviewLabel}${reviewNextLabel}${scopeLabel})`;
  }
  if (state?.approved) return `PF gate: approved (scope v${state.scopeVersion}${modeLabel}${reviewLabel}${reviewNextLabel}${scopeLabel})`;
  return `PF gate: waiting approval (scope v${Math.max(1, state?.scopeVersion || 1)}${modeLabel}${reviewLabel}${reviewNextLabel}${scopeLabel})`;
}
