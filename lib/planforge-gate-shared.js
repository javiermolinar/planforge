...
function gateStatusLine(state) {
  ...
  if (mode === "fast") return `PF gate: fast mode active${modeLabel}${profileLabel}${reviewLabel}${reviewNextLabel}`;
  ...
  return `PF gate: waiting approval (scope v${Math.max(1, state?.scopeVersion || 1)}${modeLabel}${profileLabel}${reviewLabel}${reviewNextLabel})`;
}
